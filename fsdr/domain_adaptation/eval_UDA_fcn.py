...
from advent.utils.func import per_class_iu, fast_hist, f_select
...
def eval_best(cfg, models,
    ...
    for i_iter in range(start_iter, max_iter + 1, step):
        ...
        if verbose:
            if cfg.TEST.TEST_ON_GTA:
                display_stats(cfg, cfg.TEST.class_names_gta, inters_over_union_classes)
            else:
                display_stats(cfg, test_loader.dataset.class_names, inters_over_union_classes)


def load_checkpoint_for_evaluation(model, checkpoint, device):
    saved_state_dict = torch.load(checkpoint, map_location='cuda:2')
